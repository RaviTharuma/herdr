...
Actions that declare `contexts = ["pane"]`, `["tab"]`, or `["workspace"]` also
appear on the matching right-click context menu when the plugin is enabled and
supports the current platform. Choosing a plugin row focuses that target, then
invokes the action with `invocation_source = "context_menu"` and the usual
`HERDR_PLUGIN_CONTEXT_JSON` fields. Empty `contexts` keeps the action
keybind/CLI-only and does not add a menu row.
